...
TEST(TorrentPreview, RejectsMissingTorrentFile) {
    ...
    const std::string err =
        torrex::preview_torrent_file("/nonexistent/torrex-missing.torrent", preview);
...
TEST(TorrentPreview, RejectsInvalidMagnet) {
    torrex::TorrentAddPreview preview;
    const std::string err = torrex::preview_magnet_uri("not-a-magnet", preview);
    EXPECT_FALSE(err.empty());
}

TEST(TorrentPreview, ParsesMagnetDisplayName) {
    torrex::TorrentAddPreview preview;
    const std::string err = torrex::preview_magnet_uri(
